...
#include <ferrari_vehicle/msg/odom_lite.h>
...
public:
    ...
    PublisherOdom(float rate_hz = 30.0f)
    {
        ...
        memset(&odom_msg_, 0, sizeof(ferrari_vehicle__msg__OdomLite));
    ...
    void init(rcl_node_t *node) override
    {
        ...
        RCCHECK(rclc_publisher_init_best_effort(
            getPublisher(),
            node,
            ROSIDL_GET_MSG_TYPE_SUPPORT(ferrari_vehicle, msg, OdomLite),
            "/vehicle/odom_lite"));
    }
    ...
    void publish() override
    {
        ...
        // Position
        odom_msg_.pose.position.x = odom_state.x;
        odom_msg_.pose.position.y = odom_state.y;
        odom_msg_.pose.position.z = 0.0;

        // Orientation
        odom_msg_.pose.orientation.x = 0.0f;
        odom_msg_.pose.orientation.y = 0.0f;
        odom_msg_.pose.orientation.z = sinf(odom_state.theta / 2.0f);
        odom_msg_.pose.orientation.w = cosf(odom_state.theta / 2.0f); // Use cosf instead of cos

        float magnitude = sqrtf((odom_msg_.pose.orientation.x * odom_msg_.pose.orientation.x) +
                                (odom_msg_.pose.orientation.y * odom_msg_.pose.orientation.y) +
                                (odom_msg_.pose.orientation.z * odom_msg_.pose.orientation.z) +
                                (odom_msg_.pose.orientation.w * odom_msg_.pose.orientation.w));

        if (magnitude < 1e-6f)
        {
            odom_msg_.pose.orientation.x = 0.0f;
            odom_msg_.pose.orientation.y = 0.0f;
            odom_msg_.pose.orientation.z = 0.0f;
            odom_msg_.pose.orientation.w = 1.0f;
        }
        else
        {
            // 4. Normalize components safely
            odom_msg_.pose.orientation.x /= magnitude;
            odom_msg_.pose.orientation.y /= magnitude;
            odom_msg_.pose.orientation.z /= magnitude;
            odom_msg_.pose.orientation.w /= magnitude;
        }

        // Twist / Velocity
        odom_msg_.twist.linear.x = odom_state.linear_x;
        odom_msg_.twist.angular.z = odom_state.angular_z;

        // SerialUSB1.print("Publishing odometry...");
...
private:
    ...
    ferrari_vehicle__msg__OdomLite odom_msg_;
};